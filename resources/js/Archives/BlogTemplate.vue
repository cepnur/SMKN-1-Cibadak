  ...
  <section>
    <div class="container mx-auto" v-if="posts && posts.length > 0">
      ...
      <div class="articles-feed grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5" v-if="posts && posts.length > 0">
        <a v-for="(post, i) in posts" :key="i" :href="post.link" class="flex flex-col gap-5 border-b md:border-b-0 border-primary pb-5">
          <LazyImage :alt="post.title" aria-label="Read more about {{post.title}}" :src="post.image" width="300px" height="300px" style="height: 300px" className="object-cover w-full" />
          <article class="article-detail">
            <span
              class="date text-sm block mt-5 mb-3"
              :class="{
      ...
      <div class="flex justify-center gap-x-5 mt-3 mb-10 justify-between">
        ...
        <a
          ...
          :class="{
            'opacity-0 hidden invisible pointer-events-none': !pagination.has_next_page,
          }"
          title="Next Page"
